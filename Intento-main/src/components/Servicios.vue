...
const categorias = ref<Categoria[]>([
  { id: 1, nombre: 'Cabello',  descripcion: '' },
  { id: 2, nombre: 'Uñas',     descripcion: '' },
  { id: 3, nombre: 'Facial',   descripcion: '' },
])

const servicios = ref<Servicio[]>([
  { id: 1, nombre: 'Corte de Cabello', categoriaId: 1, duracion: 45, precio: 150, descripcion: 'Corte personalizado dama y caballero', estado: 'activo' },
  { id: 2, nombre: 'Tinte Completo',   categoriaId: 1, duracion: 120, precio: 450, descripcion: 'Coloración completa con productos de calidad', estado: 'activo' },
  { id: 3, nombre: 'Manicure',         categoriaId: 2, duracion: 60, precio: 120, descripcion: 'Cuidado y esmaltado de uñas', estado: 'activo' },
])
...
function onGuardar(form: ServicioForm): void {
  if (modalAction.value === 'crear') {
    servicios.value.push({ id: Date.now(), ...form })
  } else {
    const idx = servicios.value.findIndex((s: Servicio) => s.id === selectedServicio.value?.id)
    if (idx !== -1) {
      servicios.value[idx] = { id: servicios.value[idx]!.id, ...form }
    }
  }
  ...
  <div style="position:fixed;inset:0;display:flex;font-family:'Jost',sans-serif;background:#F0E8DC;overflow:hidden;">
    ...
    <main style="flex:1;display:flex;flex-direction:column;padding:28px 32px;min-width:0;position:relative;z-index:5;overflow:hidden;">
      ...
      <div style="flex:1;background:white;border-radius:20px;box-shadow:0 8px 40px rgba(122,78,56,0.1);overflow:auto;">
        ...
        <!-- Empty state -->
        <div v-if="serviciosFiltrados.length === 0" style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:80px 20px;color:#A896A0;">
        ...
        <table v-else style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          <tbody>
            <tr v-for="s in serviciosFiltrados" :key="s.id"
              ...
              <!-- Duración -->
              <td style="padding:16px 20px;vertical-align:middle;color:#5a4040;font-size:13px;text-align:left;">{{ s.duracion }} min</td>

              <!-- Precio -->
              <td style="padding:16px 20px;vertical-align:middle;font-weight:600;color:#7A4E38;font-size:13px;text-align:left;">${{ s.precio }}</td>

              <!-- Descripción -->
              <td style="padding:16px 20px;vertical-align:middle;color:#5a4040;font-size:13px;text-align:left;">{{ s.descripcion }}</td>
