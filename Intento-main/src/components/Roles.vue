...
const roles = ref<Rol[]>([
  ...
  { id: 2, nombre: 'Recepcionista', descripcion: 'Gestión de citas y clientes', estado: 'activo' },
  { id: 3, nombre: 'Estilista',     descripcion: 'Visualización de citas asignadas', estado: 'activo' },
])
...
const rolesFiltrados = computed<Rol[]>(() => {
  ...
  return roles.value.filter((r: Rol) =>
...
function onGuardar(form: RolForm): void {
  if (modalAction.value === 'crear') {
    roles.value.push({ id: Date.now(), ...form })
  } else {
    const idx = roles.value.findIndex((r: Rol) => r.id === selectedRol.value?.id)
    if (idx !== -1) {
      roles.value[idx] = { id: roles.value[idx]!.id, ...form }
    }
  }
    ...
    <main style="flex:1;display:flex;flex-direction:column;padding:28px 32px;min-width:0;position:relative;z-index:5;overflow:hidden;">
      ...
      <header style="display:flex;align-items:center;gap:12px;margin-bottom:24px;flex-wrap:wrap;">
        <div style="text-align:left;">
          <h2 style="font-family:'Cormorant Garamond',Georgia,serif;font-size:30px;font-weight:700;color:#7A4E38;margin:0;line-height:1.1">Gestión de Roles</h2>
          <p style="font-size:12px;color:#A896A0;margin:3px 0 0">{{ roles.length }} roles registrados</p>
        </div>
      ...
      <div style="flex:1;background:white;border-radius:20px;box-shadow:0 8px 40px rgba(122,78,56,0.1);overflow:auto;">
        ...
        <!-- Empty state -->
        <div v-if="rolesFiltrados.length === 0" style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:80px 20px;color:#A896A0;">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#D4A8B0" stroke-width="1.2" style="margin-bottom:12px"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
        ...
        <table v-else style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          <tbody>
            <tr v-for="r in rolesFiltrados" :key="r.id"
              ...
              <td style="padding:16px 20px;vertical-align:middle;">
                <div style="display:flex;align-items:center;gap:12px;">
                  ...
                  <span style="font-weight:600;color:#7A4E38;font-size:13px;">{{ r.nombre }}</span>
                </div>
              </td>

              <!-- Descripción -->
              <td style="padding:16px 20px;vertical-align:middle;color:#5a4040;font-size:13px;text-align:left;">{{ r.descripcion }}</td>

              <!-- Estado -->
              <td style="padding:16px 20px;vertical-align:middle;text-align:center;">
                <span :style="{
                  background: r.estado === 'activo' ? 'rgba(134,188,149,0.15)' : 'rgba(212,168,176,0.2)',
              ...
              <td style="padding:16px 20px;vertical-align:middle;">
                <div style="display:flex;align-items:center;justify-content:center;gap:8px;">
                  <button @click="openEditar(r)" title="Editar"
