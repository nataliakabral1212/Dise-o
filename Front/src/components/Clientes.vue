...
const clientesFiltrados = computed<Cliente[]>(() => {
  ...
  return clientes.value.filter((c: Cliente) =>
...
async function cargarClientes(): Promise<void> {
  ...
  try {
    ...
    clientes.value = data.map((c: any) => ({ ...c, createdAt: c.created_at }))
  } catch (e) {
...
async function crearCliente(form: ClienteForm): Promise<void> {
  const res = await fetch(`${BASE_URL}/clientes`, {
    method: 'POST',
  ...
  const data = await res.json()
  clientes.value.unshift({ ...data.data, createdAt: data.data.created_at })
}
...
async function onGuardar(form: ClienteForm): Promise<void> {
  try {
...
function onEliminar(): void {
  if (clienteAEliminar.value) {
    clientes.value = clientes.value.filter((c: Cliente) => c.id !== clienteAEliminar.value!.id)
  }
    ...
    <main style="flex:1;display:flex;flex-direction:column;padding:28px 32px;min-width:0;position:relative;z-index:5;overflow:hidden;">
      <header style="display:flex;align-items:center;gap:12px;margin-bottom:24px;flex-wrap:wrap;">
        <div style="text-align:left;">
          <h2 style="font-family:'Cormorant Garamond',Georgia,serif;font-size:30px;font-weight:700;color:#7A4E38;margin:0;line-height:1.1">Gestión de Clientes</h2>
          <p style="font-size:12px;color:#A896A0;margin:3px 0 0">{{ clientes.length }} clientes registrados</p>
        </div>
      ...
      <div style="flex:1;background:white;border-radius:20px;box-shadow:0 8px 40px rgba(122,78,56,0.1);overflow:auto;">
        ...
        <div v-else-if="clientesFiltrados.length === 0" style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:80px 20px;color:#A896A0;">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#D4A8B0" stroke-width="1.2" style="margin-bottom:12px"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
          <p style="font-size:14px;">No se encontraron clientes</p>
        </div>

        <table v-else style="width:100%;border-collapse:collapse;font-size:13px;">
          <colgroup>
            <col style="width:28%" /><col style="width:27%" /><col style="width:18%" /><col style="width:18%" /><col style="width:9%" />
          </colgroup>
          <thead>
            <tr style="background:linear-gradient(135deg,#D4A8B0 0%,#C4A882 55%,#9A7A5A 100%);">
              <th style="padding:14px 20px;text-align:left;color:white;font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;">Cliente</th>
              ...
              <th style="padding:14px 20px;text-align:left;color:white;font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;">Fecha Registro</th>
          ...
          <tbody>
            <tr v-for="c in clientesFiltrados" :key="c.id"
              ...
              <td style="padding:16px 20px;vertical-align:middle;color:#5a4040;font-size:13px;">{{ c.correo }}</td>
              <td style="padding:16px 20px;vertical-align:middle;color:#5a4040;font-size:13px;">{{ c.telefono }}</td>
              <td style="padding:16px 20px;vertical-align:middle;color:#A896A0;font-size:13px;">{{ fmtDate(c.createdAt) }}</td>
              <td style="padding:16px 20px;vertical-align:middle;">
